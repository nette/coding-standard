...
# Use additional config file (resolved by extension: .php → Fixer, .xml → Sniffer)
./ecs fix src --config-file ./my-overrides.php

# Both tools at once (specify --config-file twice)
./ecs fix src --config-file ./my-overrides.php --config-file ./my-overrides.xml
...
- `run.php` - Main orchestrator:
  - Parses CLI arguments (`check`, `fix`, `--preset`, `--config-file`, paths)
  - Validates `--config-file` path/extension, propagates the `.php` variant via env `NCS_CONFIG_FILE_PHP` and the `.xml` variant via `Checker` constructor argument
...
- `src/Checker.php` - Core logic:
  ...
  - Builds a temporary wrapper ruleset for the sniffer that always references the version preset and (if present) the project's `ncs.xml` plus an optional CLI-supplied xml. Source files are never modified – `$presets/` substitution happens in temp copies.
  - Constructor takes optional `?string $snifferConfig` for the CLI-supplied sniffer ruleset path
...
- Whitelist: `includedFunctions` and `includedConstants` (wildcard patterns) - when non-empty, only matching symbols are imported
- Blacklist: `excludedFunctions` and `excludedConstants` (wildcard patterns) - always applied on top to exclude symbols
- Resolution: includedX → optimizedFunctionsOnly → excludedX
...
- Adds `OptimizeGlobalCalls` sniff with:
  ...
  - Excluded functions: `dump`, `var_dump`, `print_r`, `error_get_last`, `trigger_error`, `debug_backtrace`
  - Included constants: `PHP_*`, `DIRECTORY_SEPARATOR`
...
Projects can override rules in two ways (and CLI `--config-file` can add a third layer on top).
...
```
These rules are merged with preset rules (custom rules take precedence). The file is located by walking up from CWD looking for `ncs.php`.

**2. Custom PHP CodeSniffer Ruleset (ncs.xml):**
Create `ncs.xml` in project root (it does NOT need to reference the preset; the wrapper adds it automatically):
```xml
<?xml version="1.0"?>
...
Legacy `ncs.xml` files that still contain `<rule ref="$presets/phpXX.xml"/>` continue to work (phpcs merges duplicate refs).

**Important:** `$presets/` in an xml ruleset is substituted with the absolute `preset-sniffer/` path in a temp copy (`ruleset-*.tmp.xml`); the original file is never modified.

**3. Additional CLI config via `--config-file <path>`:**
```bash
./ecs fix --config-file ./ci-overrides.php
./ecs fix --config-file ./ci-overrides.xml
./ecs fix --config-file ./ci-overrides.php --config-file ./ci-overrides.xml
```
- Tool is picked by extension: `.php` → Fixer, `.xml` → Sniffer. Other extensions are rejected.
- Automatic discovery of `ncs.php`/`ncs.xml` still runs; the CLI file is applied **on top** and its values take precedence.
- Typical use case: temporarily disable a rule without committing an `ncs.php` to the project, e.g.:
  ```php
  <?php
  // my-overrides.php
  return ['PhpCsFixerCustomFixers/commented_out_function' => false];
  ```
  Running `./ecs fix src --config-file ./my-overrides.php` will stop `dump()`/`var_dump()` calls from being commented out.
...
- Creates `filelist.tmp` with all matched file paths. Used by both PHP CS Fixer and PHP CodeSniffer (`--file-list`).
- Creates per-source `ruleset-<tag>.tmp.xml` files (substituted copies of project/CLI rulesets) and a `ruleset-wrapper.tmp.xml` that references them in order (preset → project → CLI).
- All temp files are removed by `Checker::cleanup()`, including on Ctrl+C via the signal handler.
...
3. **Custom ruleset path replacement**: `$presets/` in `ncs.xml` (or a `--config-file *.xml`) is replaced at runtime with the absolute preset-sniffer path. Substitution happens in a temp copy; the source file is never modified. The project's `ncs.xml` no longer needs to reference the version preset – the wrapper ruleset includes it automatically.

4. **File list dependency**: Both tools rely on `filelist.tmp` - if Checker fails before creating it, both tools will fail.

5. **Signal handling**: Ctrl+C is caught and triggers cleanup (removes `filelist.tmp` and all `ruleset-*.tmp.xml`).
