...
class OptimizeGlobalCallsSniff implements Sniff
{
	...
	private function findUsedGlobalFunctions(File $phpcsFile, array $existingUseStatements): array
	{
		...
		for ($i = 0; $i < $phpcsFile->numTokens; $i++) {
			...
			// Handle T_NAME_FULLY_QUALIFIED tokens (\strlen, \count, etc.)
			if ($tokens[$i]['code'] === T_NAME_FULLY_QUALIFIED) {
				$content = $tokens[$i]['content'];
				if ($content[0] !== '\\' || substr_count($content, '\\') !== 1) {
					continue;
				}
				$functionName = substr($content, 1);
			...
			// Handle old-style T_NS_SEPARATOR + T_STRING tokens
			if ($tokens[$i]['code'] !== T_STRING) {
				continue;
			}

			if ($this->isFunctionDeclaration($phpcsFile, $i)) {
				continue;
			}
	...
	private function findUsedGlobalConstants(File $phpcsFile, array $existingUseStatements): array
	{
		...
		for ($i = 0; $i < $phpcsFile->numTokens; $i++) {
			...
			// Handle T_NAME_FULLY_QUALIFIED tokens (\PHP_VERSION, etc.)
			if ($tokens[$i]['code'] === T_NAME_FULLY_QUALIFIED) {
				$content = $tokens[$i]['content'];
				if ($content[0] !== '\\' || substr_count($content, '\\') !== 1) {
					continue;
				}
				$constantName = substr($content, 1);
	...
	private function removeBackslashesFromCode(File $phpcsFile, array $usedFunctions, array $usedConstants): void
	{
		...
		for ($i = 0; $i < $phpcsFile->numTokens; $i++) {
			// Handle T_NAME_FULLY_QUALIFIED tokens (\strlen, \PHP_VERSION, etc.)
			if ($tokens[$i]['code'] === T_NAME_FULLY_QUALIFIED) {
				$content = $tokens[$i]['content'];
				if ($content[0] !== '\\' || substr_count($content, '\\') !== 1) {
					continue;
	...
	private function hasBackslashesToRemove(File $phpcsFile, array $usedFunctions, array $usedConstants): bool
	{
		...
		for ($i = 0; $i < count($tokens); $i++) {
			// Handle T_NAME_FULLY_QUALIFIED tokens
			if ($tokens[$i]['code'] === T_NAME_FULLY_QUALIFIED) {
				$content = $tokens[$i]['content'];
				if ($content[0] !== '\\' || substr_count($content, '\\') !== 1) {
					continue;
