...
',
	...
	protected function applyFix(\SplFileInfo $file, Tokens $tokens): void
	{
		...
		foreach ($tokens as $index => $token) {
			...
			if (
				...
				if ($scopes[$currentScope]['type'] === 'block_signature') {
				...
				if ($this->bracesFixerCompatibility) {
					...
					if ($prevIndex !== null && $tokens[$prevIndex]->isGivenKind([T_FUNCTION, T_FN])) {
						$skip = true;
					}
				}
			...
			if (
				...
				if ($scopes[$currentScope]['type'] === 'block' || $scopes[$currentScope]['type'] === 'block_signature') {
					$indent = false;
					...
					if ($scopes[$currentScope]['is_indented_block']) {
						...
						for ($searchIndex = $index + 1, $max = \count($tokens); $searchIndex < $max; ++$searchIndex) {
							...
							if (!$searchToken->isWhitespace()) {
								if ($firstNonWhitespaceTokenIndex === null) {
						...
						if (
							($firstNonWhitespaceTokenIndex !== null && $firstNonWhitespaceTokenIndex < $endIndex)
							|| ($nextNewlineIndex !== null && $nextNewlineIndex < $endIndex)
						) {
							if (
								...
								if (
									...
									if ($nextNextIndex !== null && $tokens[$nextNextIndex]->isGivenKind([T_ELSE, T_ELSEIF])) {
			...
			if ($scopes[$currentScope]['type'] !== 'statement' && $scopes[$currentScope]['type'] !== 'block_signature') {
	...
	private function findStatementEndIndex(Tokens $tokens, int $index, int $parentScopeEndIndex): int
	{
		...
		for ($searchEndIndex = $index; $searchEndIndex <= $parentScopeEndIndex; ++$searchEndIndex) {
			...
			if (
				$ifLevel > 0
				&& $controlStructureContinuationIndex !== null
	...
	private function isCommentWithFixableIndentation(Tokens $tokens, int $index): bool
	{
		...
		while (true) {
			...
			if ($firstCommentCandidateIndex === null) {
				break;
			}
		...
		while (true) {
			...
			if ($lastCommentCandidateIndex === null) {
				break;
			}
