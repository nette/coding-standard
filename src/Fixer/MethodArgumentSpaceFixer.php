...
final class MethodArgumentSpaceFixer extends AbstractFixer implements ConfigurableFixerInterface, WhitespacesAwareFixerInterface
{
	...
	private function fixFunction(Tokens $tokens, int $startFunctionIndex): bool
	{
		...
		foreach ([$firstWhitespaceIndex, $lastWhitespaceIndex] as $index) {
			if ($index === null || !Preg::match('/\R/', $tokens[$index]->getContent())) {
				continue;
			}

			if ($this->configuration['on_multiline'] !== 'ensure_single_line') {
				$isMultiline = true;
	...
	private function findWhitespaceIndexAfterParenthesis(
		Tokens $tokens,
	...
	private function ensureFunctionFullyMultiline(Tokens $tokens, int $startFunctionIndex): void
	{
		...
		} while ($prevWhitespaceTokenIndex !== null
			&& !str_contains($tokens[$prevWhitespaceTokenIndex]->getContent(), "\n")
		);

		if ($prevWhitespaceTokenIndex === null) {
			$existingIndentation = '';
			...
			$existingIndentation = $lastLineIndex === false
				? $existingIndentation
		...
		for ($index = $endFunctionIndex - 1; $index > $startFunctionIndex; --$index) {
			...
			if ($token->isGivenKind(CT::T_ATTRIBUTE_CLOSE)) {
				if ($this->configuration['attribute_placement'] === 'standalone') {
					$this->fixNewline($tokens, $index, $indentation);
				} elseif ($this->configuration['attribute_placement'] === 'same_line') {
	...
	private function fixSpace(Tokens $tokens, int $index): void
	{
		...
		if ($tokens[$index - 1]->isWhitespace()) {
			...
			if (
				...
				&& ($this->configuration['after_heredoc'] === true || !$tokens[$prevIndex]->isGivenKind(T_END_HEREDOC))
			) {
		...
		if ($nextToken->isWhitespace()) {
			...
			if (
				($this->configuration['keep_multiple_spaces_after_comma'] === false || Preg::match('/\R/', $newContent))
