		...
		foreach ($tokens as $index => $token) {
			...
			if ($token->isGivenKind($classyTokens)) {
				...
				if ($tokensAnalyzer->isAnonymousClass($index)) {
					$allowSingleLineIfEmpty = $this->configuration['allow_single_line_empty_anonymous_classes'] === true;
				...
				if ($tokensAnalyzer->isLambda($index)) {
					$allowSingleLine = $this->configuration['allow_single_line_anonymous_functions'] === true;
			...
			if ($allowSingleLine || $allowSingleLineIfEmpty || $index < $allowSingleLineUntil) {
				...
				if (!$addNewlinesInsideBraces && $allowSingleLineUntil === null) {
			...
			$whitespace = ' ';
			if ($this->configuration[$positionOption] === self::NEXT_LINE_UNLESS_NEWLINE_AT_SIGNATURE_END) {
			...
			if ($whitespace === ' ') {
				...
				!$tokens[$openBraceIndex - 1]->isWhitespace()
				|| !Preg::match('/\R/', $tokens[$openBraceIndex - 1]->getContent())
			) {
				for ($indexAfterOpenBrace = $openBraceIndex + 1; $indexAfterOpenBrace < $closeBraceIndex; ++$indexAfterOpenBrace) {
					if (
						$tokens[$indexAfterOpenBrace]->isWhitespace()
						&& Preg::match('/\R/', $tokens[$indexAfterOpenBrace]->getContent())
					) {
						break;
					}

					if (
						$tokens[$indexAfterOpenBrace]->isComment()
						&& !str_starts_with($tokens[$indexAfterOpenBrace]->getContent(), '/*')
					) {
			...
			if ($moveBraceToIndex !== null) {
				...
				for (; $openBraceIndex !== $moveBraceToIndex; $openBraceIndex += $delta) {
					...
					\assert($siblingToken instanceof Token);
